...
function InlineSetTimer({ remaining, onSkip, lang }) {
  return (
    <div
    ...
    >
      <span style={{ fontWeight: 600 }}>
        {lang === "pt" ? "Descanso" : "Rest"}: {remaining}s
      </span>
    ...
    <div className="card" style={allDone ? { borderColor: "var(--accent)" } : undefined}>
      <div className="flex items-center justify-between mb-3 gap-2">
        ...
        <div className="flex items-center gap-1">
          <button className="btn btn-ghost p-1.5" onClick={() => setShowNote((v) => !v)}>
            <StickyNote size={14} style={{ color: note ? "var(--accent)" : "var(--muted)" }} />
          </button>
          <button className="btn btn-ghost p-1.5" onClick={() => onRemoveExercise(exIdx)}>
      ...
      {showNote && (
        <textarea
          ...
          rows={2}
          style={{ resize: "none" }}
          placeholder={lang === "pt" ? "Nota para este exercício..." : "Note for this exercise..."}
          value={note}
      ...
      <div
        ...
        style={{ gridTemplateColumns: "22px minmax(0,1fr) minmax(0,1fr) 28px" }}
      >
        <span className="text-xs text-center" style={{ color: "var(--muted)" }}>#</span>
        <span className="text-xs text-center" style={{ color: "var(--muted)" }}>kg</span>
        <span className="text-xs text-center" style={{ color: "var(--muted)" }}>reps</span>
        <span />
      </div>
      ...
      {exercise.sets.map((set, setIdx) => (
        <div key={set.id}>
          ...
          {setTimer?.exIdx === exIdx && setTimer?.setIdx === setIdx && (
            <InlineSetTimer remaining={setTimer.remaining} onSkip={onSkipSetTimer} lang={lang} />
          )}
        </div>
      ))}
